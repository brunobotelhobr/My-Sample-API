...
function sendingRequest(msg, initiator, helper) {
  ...
  if (initiator !== HttpSender.AUTHENTICATION_INITIATOR && msg.isInScope()) {
    msg.getRequestHeader().setHeader("Authorization", "Bearer " + ScriptVars.getGlobalVar("jwt-token"));
    logger("Added Authorization header to request", "Authorization", "Bearer " + ScriptVars.getGlobalVar("jwt-token"));
